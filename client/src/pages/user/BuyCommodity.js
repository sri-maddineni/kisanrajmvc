...
const BuyCommodity = () => {
    ...
    const handleDecline = async (pid, sellerid) => {
        try {
            const buyerid = auth?.user._id;
            const { data } = await axios.post(`${process.env.REACT_APP_API}/api/v1/products/decline`, { buyerid, pid, sellerid });
            if (data?.success) {
        ...
        <>
                            ...
                            <div className="col-lg-3 col-md-3 col-sm-6 m-4" key={p._id}>
                                <div className="card" style={{ width: "18rem" }}>
                                    ...
                                    <div className="card-body">
                                        <h5 className="card-title" style={{ fontSize: "1rem" }}>{p.organic ? "Organic" : "Inorganic"} {p.name} {p.quality} <i className="fa-solid fa-star text-warning"></i><br />{p.description}</h5>
                                        <p className="card-text" style={{ fontSize: "1rem" }}>
                                            <span className='text-dark bg-warning'>
                                                Rs.{p.price}/- per {p.quantity} {p.quantityUnit}s
                                                <br />
                                                (Rs. {(p.price / p.quantity).toFixed(1)} per {p.quantityUnit})
                                            </span>
                                        </p>
                                        ...
                                        <i className="fa-brands fa-whatsapp mx-2" style={{ cursor: "pointer" }} ></i>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <Modal
                ...
                footer={[
                    ...
                    <Button key="submit" type="primary" onClick={() => handleOk(selectedProduct._id, selectedProduct.sellerId._id)}>Submit</Button>,
                ]}
            >
                <p>Product Name: {selectedProduct?.name}</p>
                <div className="p-1 m-1" >
                    <input type="number" required={true} value={quantity} onChange={(e) => setquantity(e.target.value)} style={{ borderRadius: "5px" }} className="p-1 m-1" placeholder="Required quantity" /> <label htmlFor="">{selectedProduct?.quantityUnit}s</label>

                </div>
                <div className="p-1 m-1">
                    Rs. <input type="number" required={true} value={price} onChange={(e) => setprice(e.target.value)} style={{ borderRadius: "5px" }} className="p-1 m-1" placeholder="Offered price" /> per {quantity} {selectedProduct?.quantityUnit}s
                </div>

                <div className="p-1 m-1" >
                    Required date: <input type="date" required={true} value={date} onChange={(e) => setdate(e.target.value)} style={{ borderRadius: "5px" }} className="p-1 m-1" placeholder="Required date" />
                </div>
