...
const Responses = () => {
    ...
    const [product, setProduct] = useState({}); //for product details maintainance


    const [requirements, setRequirements] = useState([]);   //to fetch all requirements from requirements model


    const [auth, setAuth] = useContext(AuthContext);//to maintain user state and login details for profile page
